with order_subtotals as (
    ...
    from analytics.dbt_cbuckley.int_valid_order_refs as orders
    inner join analytics.dbt_cbuckley.int_order_line_inputs as lines
...
promo_lines as (
    ...
    left join analytics.dbt_cbuckley.stg_promotions as promo
        on promo.promo_code = order_subtotals.promo_code
    left join analytics.dbt_cbuckley.int_order_line_inputs as lines
        on lines.external_order_ref = order_subtotals.external_order_ref
    left join analytics.dbt_cbuckley.int_product_master as product
