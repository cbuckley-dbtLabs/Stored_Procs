  ...
  create or replace   view analytics.dbt_cbuckley_stored_proc.int_order_line_inputs
  ...
  as (
    with valid_lines as (
    select raw.*
    from analytics.dbt_cbuckley_stored_proc.int_raw_orders_validated as raw
    inner join analytics.dbt_cbuckley_stored_proc.int_valid_order_refs as orders
...
price_list_prices as (
    ...
    inner join analytics.dbt_cbuckley_stored_proc.stg_price_lists as list
    ...
    inner join analytics.dbt_cbuckley_stored_proc.stg_price_list_items as item
...
list_price_fx as (
    ...
    left join analytics.dbt_cbuckley_stored_proc.int_fx_rates as fx
...
inner join analytics.dbt_cbuckley_stored_proc.int_product_master as product
    on product.product_id = line.product_id
inner join analytics.dbt_cbuckley_stored_proc.int_customer_master as customer
